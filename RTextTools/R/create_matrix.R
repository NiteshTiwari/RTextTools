create_matrix <- function(textColumns, 
                          language="english", 
                          minDocFreq=1, 
                          maxDocFreq=Inf, 
                          minWordLength=3, 
    ...
    control <- list(bounds=list(local=c(minDocFreq,maxDocFreq)),
                    ...
                    wordLengths=c(minWordLength,maxWordLength),weighting=weighting)
    ...
    vector_source <- VectorSource(trainingColumn)
	corpus <- Corpus(vector_source, readerControl=list(language=language))
